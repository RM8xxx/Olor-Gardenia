...
// Comprehensive dictionary of exact Fragrantica / verified image URLs
const VERIFIED_IMAGE_MAP: Record<string, string> = {
  ...
  // One Direction, Penhaligon's, Roja, Thomas Kosmala
...
async function testAllUrls() {
  console.log(`Testing ${Object.keys(VERIFIED_IMAGE_MAP).length} verified URLs...`);
  let errors = 0;
  for (const [id, url] of Object.entries(VERIFIED_IMAGE_MAP)) {
    try {
      ...
      console.error(`❌ ERR ${id} -> ${url} (${e.message})`);
      errors++;
    }
  }
  console.log(`URL test complete. Errors: ${errors}`);
}
